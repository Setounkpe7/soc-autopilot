...
propre de chaque règle). La **CI échoue si ces fichiers sont périmés** par rapport aux
tags des règles Sigma (`git diff --exit-code docs/coverage/`) — la couverture ne peut
donc pas mentir : c'est un produit du code, régénéré à chaque commit.
